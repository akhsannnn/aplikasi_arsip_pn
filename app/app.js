const app = {
  ...
  // ... (Bagian atas app.js tetap sama) ...
    ...
    renderBreadcrumbs: function(crumbs) {
        ...
        crumbs.forEach(c => {
            html += `<li><div class="flex items-center"><i class="fa-solid fa-chevron-right text-gray-400 mx-2 text-[10px]"></i><a href="#" onclick="app.openFolder(${c.id})" class="text-gray-600 hover:text-court-green text-xs font-medium uppercase hover:underline">${c.name}</a></div></li>`;
        });
    ...
    this.showConfirm(
        'Hapus Folder?',
        'Folder beserta isinya akan dihapus.',
        'danger',
        async () => {
            const fd = new FormData();
            fd.append('id', id);

            const res = await this.postData('delete_folder', fd);
    ...
    openGenerateModal: function() { 
        // Redirect ke halaman template selection untuk user memilih template
        window.location = `generate.php?action=select&year=${this.year}`;
    },
    ...
    previewFile: function(path, name, type) {
        ...
        if (type == 'pdf') {
            cont.innerHTML = `<iframe src="${path}" class="w-full h-full border-none"></iframe>`;
        } else if (type == 'mp4' || type == 'avi' || type == 'mov' || type == 'mkv' || type == 'webm') {
            cont.innerHTML = `
                <video controls class="max-w-full max-h-full shadow-lg">
                    <source src="${path}" type="video/${type == 'mp4' ? 'mp4' : type == 'avi' ? 'avi' : type == 'mov' ? 'quicktime' : type == 'mkv' ? 'x-matroska' : 'webm'}">
                    Browser Anda tidak mendukung pemutaran video.
                </video>
            `;
        } else if (type == 'jpg' || type == 'jpeg' || type == 'png' || type == 'gif' || type == 'bmp' || type == 'webp') {
            cont.innerHTML = `<img src="${path}" class="max-w-full max-h-full shadow-lg">`;
        } else {
            cont.innerHTML = `<div class="text-center text-gray-500">
                <i class="fa-solid fa-file text-6xl mb-4"></i>
                <p>Preview tidak tersedia untuk tipe file ini</p>
                <p class="text-sm">Tipe: ${type}</p>
            </div>`;
        }
